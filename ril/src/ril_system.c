...
static s32 ATResponse_QENG_Handler(char *line, u32 len, void *param)
{
	...
	p1 = Ql_RIL_FindString(line, len, "+QENG:");
	if (p1)
	{
		//p2 = Ql_strstr(p1 + 2, "\r\n");
		//if (p2)
		//{
		Ql_strcpy((char *)param,p1+9);
			//Ql_memcpy((char *)param, p2 + 2, p2 - p1 - 2);
...
static s32 ATResponse_CCID_Handler(char *line, u32 len, void *param)
{
	char *p1 = NULL;
	char *p2 = NULL;
	//char *str = (char *)param;
	...
	p1 = Ql_RIL_FindLine(line, len, "+QCCID:");
	if (p1)
	{
		return RIL_ATRSP_FAILED;
	}
...
s32 RIL_GetIMSI( char *pIMSI)
{
	...
	return Ql_RIL_SendATCmd(strAT, Ql_strlen(strAT), ATResponse_IMSI_Handler, (void *)pIMSI,0);
}
...
s32 RIL_GetQENG(char* sqeng)
{
	...
	return Ql_RIL_SendATCmd(strAT, Ql_strlen(strAT), ATResponse_QENG_Handler, (void *)sqeng,0);
}

s32 RIL_GetCCID(char* ccid)
{
	char strAT[] = "AT+QCCID";
	if (NULL == ccid)
	{
	...
	return Ql_RIL_SendATCmd(strAT, Ql_strlen(strAT), ATResponse_CCID_Handler, (void *)ccid,0);
}
